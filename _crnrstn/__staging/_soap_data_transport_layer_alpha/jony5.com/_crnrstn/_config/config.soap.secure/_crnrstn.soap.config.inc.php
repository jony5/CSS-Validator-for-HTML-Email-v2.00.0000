...
if($this->return_resource_key() == crc32('BLUEHOST')){
    ...
    //
    // ADD USER ACCOUNT - BLUEHOST
    $oClient = $oSOAP_access_manager->addClient('BLUEHOST', '0364087231749672543475966333893875', '83g#k487fg5hY%@i4fs84jfmdld8!~lf;|Qkeiur84', $CRNRSTN_NUSOAP_SVC_debugMode);
...
if($this->return_resource_key() == crc32('LOCALHOST_MACBOOKTERMINAL')) {
    ...
    $oAuth_single = $oSOAP_access_manager->generate_SOAPAuthKey('LOCALHOST_MACBOOKTERMINAL', '9898e80wq8e008f8s8f80f8s0f');
    $oAuth_single->update_permissions(CRNRSTN_RESOURCE_OPENSOURCE);
    //$oAuth_single->update_permissions(CRNRSTN_LOG_EMAIL);
    //$oAuth_single->IP_exclusiveAccess($_SERVER['SERVER_ADDR']);
    //$oAuth_single->IP_exclusiveAccess('111.111.110.*');
    //$oAuth_single->IP_denyAccess('111.111.111.112');
    //$oAuth_single->override_soap_encryption_config('AES-256-CTR', 'yuiyuuuo65456789098765y', 'ripemd256', OPENSSL_RAW_DATA);
    ...
    $oClient = $oSOAP_access_manager->addClient('LOCALHOST_MACBOOKTERMINAL', '03856145387465910978456438', '7dj3m9d2m2d99dd2dm', $CRNRSTN_NUSOAP_SVC_debugMode);

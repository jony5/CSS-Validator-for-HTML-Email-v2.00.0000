...
if($oCRNRSTN_USR->receiveFormIntegrationPacket()) {
    ...
    $tmp_key = $oCRNRSTN_USR->generate_new_key(50);

    $oCRNRSTN_USR->set_session_param('DISPLAY_AUTH_KEY', $tmp_key);
    $oCRNRSTN_USR->set_session_param('RAW_VALIDATION_DATA', $tmp_validation_results);

    $tmp_score_numeric_raw = $oCRNRSTN_USR->get_session_param('SCORE_NUMERIC_RAW');
    $tmp_packet_size = $oCRNRSTN_USR->get_session_param('PACKET_BYTES_SIZE');
    $tmp_run_time = $oCRNRSTN_USR->get_session_param('WALLTIME');
    ...
    if($oCRNRSTN_USR->isset_HTTP_Param('rtime','GET')){
        ...
        $tmp_validation_results = $oCRNRSTN_USR->get_session_param('RAW_VALIDATION_DATA');

        if(strlen($tmp_validation_results) > 1){

            $oCRNRSTN_USR->set_session_param('RAW_VALIDATION_DATA','0');

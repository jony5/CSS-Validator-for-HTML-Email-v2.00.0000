...
$oDB_RESP = $oUSER->getUserHomeData($tmp_serial_handle);
...
<div data-role="page" id="myPage">
	...
	<div role="main" class="ui-content" id="myPage">
        <h3 style="padding-bottom:0px; margin-bottom:0px;">Good morning, <?php echo $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'USERS', 'FIRSTNAME_BLOB'); ?>!</h3>
        <p style="padding-top:10px; margin-top:0px;padding-bottom:0px; margin-bottom:0px;">This can be an admin managed status
            output with whatever information would be
            useful to system users. Or a rotated tip of
            the day.</p>
        <h3 style="padding-bottom:11px; margin-bottom:0px;">Your Recent Activity</h3>
        <?php
        ...
        <div class="cb_5"></div>
        <a href="<?php echo $oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR').'account/dashboard/recent/?uid='.$oCRNRSTN_ENV->oHTTP_MGR->extractData($_GET, 'uid'); ?>" data-ajax="false" style="text-decoration:none; font-weight:normal;">View all</a>
        <h3>@mentions</h3>

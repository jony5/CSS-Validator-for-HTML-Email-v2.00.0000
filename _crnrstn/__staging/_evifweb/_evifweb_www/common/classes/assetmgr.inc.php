	...
	*/
		...
		switch($this->accessOutput['ASSET_TYPE']){
            ...
            case 'DELIVERABLE':
				...
				if(isset($loadInBrowser)){
					...
					$tmp_filePath = self::$oEnv->paramTunnelDecrypt($this->accessOutput['FILE_PATH']);
	...
	private function verifyDownloadReqIntegrity(){
		...
		if($tmp_paramString==""){
			...
			self::$oLogger->captureNotice('asset_manager->verifyDownloadReqIntegrity paramTunnelDecrypt(x) FAILURE', LOG_EMERG, 'No parameters were able to be recieved due to possible data corruption.');

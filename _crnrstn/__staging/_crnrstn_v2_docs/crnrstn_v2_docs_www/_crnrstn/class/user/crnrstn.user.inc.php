  ...
  */
    ...
    public function clientSend_CRNRSTN_SOAP_REQUEST($SOAP_method, $SOAP_request, $SOAP_endpoint=NULL){
        ...
        if(!isset($SOAP_endpoint)){
            ...
            $SOAP_endpoint = $this->getEnvResource('WSDL_URI');
        ...
        if(isset($this->WSDL_cache_ttl_ARRAY[crc32($SOAP_endpoint)])){
            ...
            $WSDL_cache_ttl = $this->getEnvResource('WSDL_CACHE_TTL');
        ...
        if(isset($this->nusoap_useCURL_ARRAY[crc32($SOAP_endpoint)])){
            ...
            $nusoap_useCURL = $this->getEnvResource('NUSOAP_USECURL');

        }

        $soapClient = new crnrstn_soap_client_manager($this, $SOAP_endpoint, $WSDL_cache_ttl, $nusoap_useCURL);

        return $soapClient->sendRequest_SOAP($SOAP_method, $SOAP_request);
    ...
    */
    ...
    public function SOAP_WSDL_connectionProfile($SOAP_endpoint, $WSDL_cache_ttl, $nusoap_useCURL){
    ...
    public function SOAP_tunnelEncryptProfile($SOAP_endpoint, $secret_key_override, $cipher_override, $hmac_algorithm_override, $options_bitwise_override){
    ...
    public function electrum_initNotifications($CRNRSTN_oELECTRUM, $email_comma_delim, $notificationProfile='EMAIL', $SOAP_endpoint=NULL, $email_protocol='SMTP'){

        $CRNRSTN_oELECTRUM->initNotifications($email_comma_delim, $notificationProfile, $SOAP_endpoint, $email_protocol);
...
";
    ...
    public function properStringBreak($str, $target_break_pattern, $maxlen){

        $tmp_array = array();
        $new_str = '';
        $tmp_string_concat = '';
        $line_char_cnt = 0;
        $pos_break_pattern = strpos($str, $target_break_pattern);

        if($pos_break_pattern !== false){

            $tmp_string_ARRAY = explode($target_break_pattern, $str);
            $tmp_size = sizeof($tmp_string_ARRAY);
            for($i=0; $i<$tmp_size; $i++){

                $line_char_cnt = strlen($tmp_string_concat);
                $line_char_cnt_next = $line_char_cnt + strlen($tmp_string_ARRAY[$i]);

                if($line_char_cnt_next > $maxlen){

                    $tmp_array[] = $tmp_string_concat;
                    $tmp_string_concat = '';
                    $tmp_string_concat .= $tmp_string_ARRAY[$i].$target_break_pattern;
            ...
            //
            // NO BREAK PATTERN FOUND. HARD BREAK AT MAX LENGTH.
            $tmp_array = str_split($str, $maxlen);

...
#  CLASS VERSION :: 1.0.0
#  CLASS DATE :: Mon July 13, 2020 @ 0525hrs
...
class crnrstn_database_query {
    ...
    public function __construct($oCRNRSTN_USR) {
        ...
        try{
            ...
            if(isset($oCRNRSTN_USR)){
                ...
                $this->oSqlSelect_tracker = new crnrstn_sqlselect_tracker(self::$oCRNRSTN_USR);
                
                //
                // SERIALIZE OBJECT - LEN32
                $this->crnrstn_db_query_serial = self::$oCRNRSTN_USR->generateNewKey();
                ...
                $this->query_ttl = self::$oCRNRSTN_USR->query_ttl;      # [0 = UNLIMITED]

                self::$oEnv = self::$oCRNRSTN_USR->return_oCRNRSTN_ENV();

                //
                // INSTANTIATE LOGGER
                //$tmp_debugMode = $_SESSION['CRNRSTN_'.crc32($_SESSION['CRNRSTN_CONFIG_SERIAL'])]['CRNRSTN_DEBUG_MODE'];
                $tmp_log_silo_key_piped = $_SESSION['CRNRSTN_'.crc32($_SESSION['CRNRSTN_CONFIG_SERIAL'])]['CRNRSTN_ACTIVE_LOG_SILO'];

                $this->oLogger = new crnrstn_logging(self::$oEnv->debugMode, $this->class_name, $tmp_log_silo_key_piped);

                $this->date_created = $this->date_modified = $this->oLogger->returnMicroTime();
                $this->creator_ip = self::$oCRNRSTN_USR->returnClientIP();
                $this->modifier_ip = self::$oCRNRSTN_USR->returnClientIP();
                $this->php_sessionid = session_id();
                $this->http_accept_language = self::$oEnv->safe_getServerArrayVar('HTTP_ACCEPT_LANGUAGE');
                $this->http_user_agent = self::$oEnv->safe_getServerArrayVar('HTTP_USER_AGENT');
                $this->http_accept_encoding = self::$oEnv->safe_getServerArrayVar('HTTP_ACCEPT_ENCODING');
                $this->https = self::$oEnv->safe_getServerArrayVar('HTTPS');
                $this->remote_addr = self::$oEnv->safe_getServerArrayVar('REMOTE_ADDR');
                $this->server_port = self::$oEnv->safe_getServerArrayVar('SERVER_PORT');
                $this->remote_port = self::$oEnv->safe_getServerArrayVar('REMOTE_PORT');
                $this->http_accept_charset = self::$oEnv->safe_getServerArrayVar('HTTP_ACCEPT_CHARSET');
                $this->request_method = self::$oEnv->safe_getServerArrayVar('REQUEST_METHOD');
    ...
    public function loadPreviousRecordLookup($result_set_key, $lookup_serial){
    ...
    public function initLookupByID($result_set_key){

        $tmp_lookup_serial = self::$oCRNRSTN_USR->generateNewKey(50);
        $this->lookupSerialIndex[$result_set_key][] = $tmp_lookup_serial;
        $this->loadPreviousRecordLookup($result_set_key, $tmp_lookup_serial);

    }

    public function addLookupFieldData($result_set_key, $field_name, $field_value){
        ...
        try{
            ...
            if(!isset($this->lookupSerialIndex[$result_set_key])){
                ...
                //
                // HOOOSTON...VE HAF PROBLEM!
                throw new Exception('oCRNRSTN_USR->initLookupByID() must be called for the result set "'.$result_set_key.'" prior to adding any lookup field data via oCRNRSTN_USR->addLookupFieldData().');
                ...
                foreach($chunkArray0 as $result_field_position=>$value){
                    switch($indexCount){
                        ...
                        default:
                            ...
                            //
                            // HOOOSTON...VE HAF PROBLEM!
                            throw new Exception('retrieveDataByID is limited to 10 keys. '.$indexCount.' keys have been provided.');
    ...
    public function resultSetMerge($oQuery, $target_result_set_key, $merge_fields_piped, $merge_fields_distinct_val, $sequence_fields_piped, $sequence_fields_datatype_piped){
        ...
        try{
            ...
            $oQuery_result_set_cnt = $oQuery->returnRecordCount();
            ...
            for($oQueryRowPos=0; $oQueryRowPos<$oQuery_result_set_cnt; $oQueryRowPos++){
                ...
                if($this->returnRecordCount() > 0){
                    ...
                    if ($mergefield_pipe_pos !== false) {
                        ...
                        for($ii=0; $ii<$piped_merge_fields_cnt; $ii++) {
                            ...
                            $value = $oQuery->returnDatabaseValue($oQuery_result_set_key, $oQuery_merge_fieldname, $oQueryRowPos);

                            if($merge_fields_distinct_val){

                                //error_log('1307 query - PIPED ['.$oQuery_fakey_result_set_key.']['.$oQuery_merge_fieldname.']['.$value.']');
                                if($this->pingValueExistence($oQuery_fakey_result_set_key, $oQuery_merge_fieldname, $value)){
                ...
                if($tmp_merge_row_authorization){
                    ...
                    $current_row_count = $this->returnRecordCount();

                    for($fieldPos=0; $fieldPos<$oQuery_fieldCnt; $fieldPos++){

                        $value = $oQuery->returnDatabaseValue($oQuery_result_set_key, $fieldPos, $oQueryRowPos);
        ...
        try{
            ...
            if(!isset($piped_primary_id_fields)){
                ...
                if($tmp_lookup_field_cnt<1){
                    ...
                    //
                    // HOOOSTON...VE HAF PROBLEM!
                    throw new Exception('No lookup ID fields have been configured through oCRNRSTN_USR->addLookupFieldData() or passed into oCRNRSTN_USR->retrieveDataByID() as a PIPE delimited STRING.');
    ...
    public function retrieveDataByID($result_set_key, $piped_lookup_fieldname, $piped_lookup_id_data){
        ...
        try {
            ...
            if(!isset($piped_lookup_id_data)){
                ...
                if($tmp_lookup_id_data_cnt<1){
                    ...
                    //
                    // HOOOSTON...VE HAF PROBLEM!
                    throw new Exception('No lookup id data has been configured through oCRNRSTN_USR->addLookupFieldData() or passed into oCRNRSTN_USR->retrieveDataByID() as a PIPE delimited STRING.');
    ...
    public function returnDatabaseValue($result_set_key, $fieldName_or_fieldPosition, $pos){
        ...
        try{
            ...
            if(isset($this->resultDataByRow[$pos][$field_position])){
                ...
                $total_count = $this->returnRecordCount();
                $total_count--;

                //
                // HOOOSTON...VE HAF PROBLEM!
                throw new Exception('Unable to find any result in the requested queue position of '.$pos.', where the range of possibility is between 0 and '.$total_count.' via the targeted result set key of "'.$result_set_key.'".');
    ...
    public function pingValueExistence($result_set_key, $fieldname, $value){
        ...
        try{
            ...
            if(!isset($this->resultDataByVal[$field_position])){
                ...
                //
                // HOOOSTON...VE HAF PROBLEM!
                throw new Exception('The result set, "'.$result_set_key.'", is empty...as it has not yet been populated with any result from the database (has oCRNRSTN_USR->processQuery() been run?), but the "'.$fieldname.'" field is being checked for a value, therein.');
            ...
            //
            // SEND THIS THROUGH THE LOGGER OBJECT
            $this->oLogger->captureNotice(__METHOD__.' on server '.$_SERVER['SERVER_NAME'].' ('.$_SERVER['SERVER_ADDR'].')', LOG_NOTICE, $e->getMessage(), self::$oCRNRSTN_USR->oLog_output_ARRAY);
    ...
    public function returnRecordCount(){
    ...
    public function initialize(){
        ...
        if($pos_select !== false){
            ...
            $this->select_query = self::$oCRNRSTN_USR->stringSanitize($tmp_field_chop_array[0],'select_statement');
            ...
            for($i=0;$i<$tmp_loop_size;$i++){
                ...
                if ($pos !== false) {
                    ...
                    $tmp_single_field_array[1] = self::$oCRNRSTN_USR->stringSanitize($tmp_single_field_array[1],'select_field_name');
                    ...
                    if ($pos_dot !== false) {
                        ...
                        if ($pos_comma !== false) {
                            ...
                            $this->oLogger->captureNotice(__METHOD__.' on server '.$_SERVER['SERVER_NAME'].' ('.$_SERVER['SERVER_ADDR'].')', LOG_NOTICE, 'Unable to map query fields. Prefixing table name to the field names may resolve. Error occurred at ['.$tmp_field_chop_array[$i].']', self::$oCRNRSTN_USR->oLog_output_ARRAY);
                    ...
                    //$this->oLogger->captureNotice('oDB_RESP->mapQueryData()', LOG_NOTICE, '11788 - Shit broken here. WTF do we have.... ->'.$tmp_single_field_array[1]);
    ...
    public function load_mysqli_connection($oCRNRSTN_MySQLi){
        ...
        $this->mysqli = $oCRNRSTN_MySQLi->returnConnObject();
...
#  AUTHOR :: Jonathan 'J5' Harris <jharris@eVifweb.com>
#  CLASS VERSION :: 1.0.0
#  CLASS DATE :: Tues July 14, 2020 @ 1400hrs
    ...
    public function return_fieldNameByPosition($pos, $nullNotice=true){
        ...
        if(isset($this->select_fieldName_ARRAY[$pos])) {
            ...
            if($nullNotice){
                ...
                $this->oLogger->captureNotice(__METHOD__.' on server '.$_SERVER['SERVER_NAME'].' ('.$_SERVER['SERVER_ADDR'].')', LOG_EMERG, 'No field name is stored in position ['.$pos.'] out of a possible maximum total of ['.sizeof($this->select_fieldName_ARRAY).'].', self::$oCRNRSTN_USR->oLog_output_ARRAY);
...
#  AUTHOR :: Jonathan 'J5' Harris <jharris@eVifweb.com>
#  CLASS VERSION :: 1.0.0
#  CLASS DATE :: Thurs July 16, 2020 @ 2158hrs
...
class crnrstn_query_profile_manager {
    ...
    public function loadQueryProfile($oCRNRSTN_MySQLi, $result_handle, $batch_key, $result_set_key){

        $tmp_query_profile_serial = self::$oCRNRSTN_USR->generateNewKey(50);
    ...
    public function returnQueryProfileSerial($result_set_key){
        ...
        try{
            ...
            $this->oLogger->captureNotice('CRNRSTN :: Error Notification :: '.__METHOD__.' on server '.$_SERVER['SERVER_NAME'].' ('.$_SERVER['SERVER_ADDR'].')', LOG_EMERG, $e->getMessage(), self::$oCRNRSTN_USR->oLog_output_ARRAY);

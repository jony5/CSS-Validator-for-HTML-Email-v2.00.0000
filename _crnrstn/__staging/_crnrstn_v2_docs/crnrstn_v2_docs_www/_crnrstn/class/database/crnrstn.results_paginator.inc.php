...
#  CLASS VERSION :: 1.0.0
#  CLASS DATE :: July 29, 2020 @ 1834hrs
...
class crnrstn_results_paginator{
        ...
        try{
            ...
            if(!isset($pagination_serial)){
                ...
                if(!isset($this->current_pagination_serial)){
                    ...
                    $tmp_form_serial = 'crnrstn_'.crc32('pagination_sensation_form_'.$tmp_pagination_serial);
                    //$tmp_form_input = 'crnrstn_'.crc32('pagination_sensation_cell_selection');
                    $tmp_form_input = $this->get_pagination_variable_name($tmp_pagination_serial);

                    self::$oCRNRSTN_USR->initFormHandling($tmp_form_serial);
                    error_log('69 paginator - listen for paginate @ ['.$tmp_form_serial.']['.$tmp_form_input.']');
                    self::$oCRNRSTN_USR->addFormInputParamListener($tmp_form_serial, $tmp_form_input, true);
                    ...
                    $tmp_requested_position = self::$oCRNRSTN_USR->returnHTTP_FormIntegration_InputVal($tmp_form_input);
                    ...
                    for($i=$start_paginate; $i<$tmp_total_page_cnt; $i++){
                        ...
                        if($i==$tmp_curr_page){
                            ...
                            if($i==$start_paginate && $tmp_curr_page>$start_paginate){
                                ...
                                $tmp_cellHTML_out .= '<span class="crnrstn_paginate_cell"><a href="#" target="_self" onClick="crnrstn_'.crc32('fire_click_event').'('.$tmp_prev_page.'); return false;"><strong>&lt;</strong>&nbsp;Previous</a></span>';
                            ...
                            if($i>$start_dot && !$tmp_dot_flag){
                                ...
                                if($i>$start_dot && $i<$tmp_total_page_cnt-5){
                                    ...
                                    $tmp_cellHTML_out .= '<span class="crnrstn_paginate_cell"><a href="#" target="_self" onClick="crnrstn_'.crc32('fire_click_event').'('.$i.'); return false;">'.$i.'</a></span>';

                                }

                                if($i==$tmp_total_page_cnt-1 && $tmp_curr_page<$tmp_total_page_cnt){

                                    $tmp_cellHTML_out .= '<span class="crnrstn_paginate_cell"><a href="#" target="_self" onClick="crnrstn_'.crc32('fire_click_event').'('.$tmp_nxt_page.'); return false;">Next&nbsp;<strong>&gt;</strong></a></span>';
    ...
    public function add_pagination_passthrough_input_val($input_name, $input_value, $pagination_serial){
        ...
        $tmp_form_serial = 'crnrstn_'.crc32('pagination_sensation_form_'.$pagination_serial);
    ...
    private function return_form_html($tmp_endpoint, $tmp_transport_protocol, $tmp_form_serial, $tmp_form_input){
        ...
        function ugc_search_sync(){
         ...
         function crnrstn_'.crc32('fire_click_event').'(pagination_index){
            var input_handle = document.getElementById("'.$tmp_form_input.'");
    ...
    public function specify_pagination_variable_name($variable_name, $pagination_serial=NULL){
        ...
        if(!isset($pagination_serial)){
            ...
            if(!isset($this->current_pagination_serial)){
                ...
                $this->current_pagination_serial = self::$oCRNRSTN_USR->generateNewKey();
    ...
    public function set_pagination_protocol($transport_protocol='get', $pagination_serial=NULL){
        ...
        $http_transport_protocol = self::$oCRNRSTN_USR->stringSanitize($http_transport_protocol, 'http_protocol_simple');

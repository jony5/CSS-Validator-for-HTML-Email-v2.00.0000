...
if($oCRNRSTN_USR->receiveFormIntegrationPacket()){
    ...
    if($oCRNRSTN_USR->isValid_dataValidationCheck('POST')){
        ...
        $oQueryProfileMgr->loadQueryProfile($oCRNRSTN_MySQLi, 'UGC_SEARCH_PREP', '!jesus_is_my_dear_lord!!', 'PAGE_DATA');
        ...
        if($tmp_page_cnt>0){
            ...
            if($pos===false){
                ...
                ';
                    ...
                    $oQueryProfileMgr->loadQueryProfile($oCRNRSTN_MySQLi, 'UGC_SEARCH', '!jesus_is_my_dear_lord!', 'QUOTED_SEARCH_'.$i);
                ...
                for($i=0;$i<$tmp_cnt;$i++){
                    ...
                    for($ii=0;$ii<$tmp_result_set_cnt;$ii++){
                        $tmp_content_id = $oCRNRSTN_USR->returnDatabaseValue($oQueryProfileMgr, 'QUOTED_SEARCH_'.$i, 'PAGE_CONTENT_RAW', $ii);
                        $tmp_return_content = $oCRNRSTN_USR->returnDatabaseValue($oQueryProfileMgr, 'QUOTED_SEARCH_'.$i, 'PAGE_CONTENT_RAW', $ii);
                        $tmp_page_path = $oCRNRSTN_USR->retrieveDataByID($oQueryProfileMgr,'PAGE_DATA','CONTENT_ID', $tmp_content_id,'CONTENT_PATH');

                        //
                        // BUILD HTML OUTPUT
                        $tmp_html_out .= $oSideBitch_Usr->returnAjaxSearchResultHTML($tmp_page_path, $tmp_return_content);
                ...
                $oQueryProfileMgr->loadQueryProfile($oCRNRSTN_MySQLi, 'UGC_SEARCH', '!jesus_is_my_dear_lord!', 'PLAIN_SEARCH');
        ...
        if($oCRNRSTN_USR->isValid_dataValidationCheck('GET')){
            ...
            $oCRNRSTN_USR->captureNotice('form demo',LOG_NOTICE, 'HTTP data received...personal_preference['.$tmp_personal_preference.'] animal_type['.$tmp_animal_type.']');
...
$tmp_http_root = $oCRNRSTN_USR->getEnvResource("ROOT_PATH_CLIENT_HTTP").$oCRNRSTN_USR->getEnvResource("ROOT_PATH_CLIENT_HTTP_DIR");

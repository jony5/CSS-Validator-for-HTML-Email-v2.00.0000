...
var fullscrn_vs = '';
var showbg_demo = '0';
var overlay_profile_xml_ARRAY = new Array();
var overlay_profile = false;
...
$.when(
	...
	if($('#overlay_body').length){
		obs_client_id = getUrlParameter('obs_id');
...
*/
		...
		if(tmp_overlaymini==true || tmp_overlaymini=='true' || tmp_overlayfs==true || tmp_overlayfs=='true') {
			showbg_demo='1';
...
*/
		...
		if(tmp_overlayfs==true || tmp_overlayfs=='true'){
			...
			overlay_profile_xml_ARRAY.push(profile_xml_http);
		...
		if(overlay_profile==true || overlay_profile=='true'){
...
function syncProfileStateXML(){
	...
	if(overlay_profile=='true' || overlay_profile==true) {
		...
		if(obs_client_id.length>0){

			tmp_curr_mini_lang_pack = $('#curr_mini_lang_pack').html();
			tmp_curr_full_lang_pack = $('#curr_full_lang_pack').html();
			...
			var tmp_vs = '&mini_vs=' + mini_vs + '&fullscrn_vs=' + fullscrn_vs + '&iso_fs=' + tmp_curr_full_lang_pack +  '&iso_m=' + tmp_curr_mini_lang_pack;
			var tmp_obs = '&obs_id=' + obs_client_id + tmp_vs;
		...
		var pars = "xml=i"+tmp_obs+"&t=" + $('#timer_copy_persist').html() + "&cache_b=" + $('#cache_bust').html();
...
function syncOverlayStateXML(pos){
	...
	if(overlay_profile==true || overlay_profile=='true'){
		...
		tmp_overload_cnt = overlay_profile_xml_ARRAY.length;

		for(ol_cnt=0;ol_cnt<tmp_overload_cnt;ol_cnt++){

			var datafile = overlay_profile_xml_ARRAY[ol_cnt];
		...
		if(profileLoadLck[type]==0){
			...
			switch(type){
				...
				case 'full':
					...
					fullscrn_vs = temp_profile_param_array['master_overlay_visible_BOOL'];

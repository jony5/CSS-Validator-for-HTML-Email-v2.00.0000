...
$tmp_serial_handle = 'OBS_CLIENT_DATA';
$oDB_RESP = $oUSER->getOBSClientData($tmp_serial_handle);
...
if($oCRNRSTN_ENV->oSESSION_MGR->getSessionParam("DEVICETYPE") == "m" || $oCRNRSTN_ENV->getEnvParam('MOBILE_ONLY') == true){
    ...
    <div data-role="page" id="myPage">
        ...
        <div role="main" class="ui-content">
            ...
            <h2 style=" padding-bottom:0px; margin-bottom:0px;">Active OBS Clients</h2>
            <div class="cb_10"></div>
            ...
            for($i=0; $i<$tmp_loop_size; $i++){
                $tmp_clientid = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'OBS_CLIENT', 'OBSCLIENT_ID', $i);
                $tmp_obs_id = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'OBS_CLIENT', 'OBS_ID', $i);
                $tmp_date_modified = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'OBS_CLIENT', 'DATEMODIFIED', $i);

                echo '<li><a href="'.$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR').'dashboard/obs/client/?obs_cid='.$tmp_clientid.'" data-ajax="false">'.$tmp_obs_id.' ('.$oCRNRSTN_ENV->oFINITE_EXPRESS->incarnate('ELAPSED', $tmp_date_modified, $tmp_format_override).')</a></li>';

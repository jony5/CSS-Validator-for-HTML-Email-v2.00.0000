...
$tmp_lang_id = strtolower($oCRNRSTN_ENV->oSESSION_MGR->getSessionParam("LANGCODE"));
...
for ($iii = 0; $iii < $tmp_loop_size; $iii++) {
    ...
    $tmp_elem_content = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'LANG_ELEM', 'ELEMENT_CONTENT', $iii);
    $tmp_elem_lang_id = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'LANG_ELEM', 'LANG_ID', $iii);

    if (($tmp_elem_lang_id == $tmp_lang_id) && ($tmp_elem_component_type=='PAGE_HEADER')) {
        $tmp_current_copy = $tmp_elem_content;
        $tmp_component_id = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'LANG_ELEM', 'COMPONENT_ID', $iii);
        $tmp_copy_id = $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'LANG_ELEM', 'COPY_ID', $iii);
    }
...
if($oCRNRSTN_ENV->oSESSION_MGR->getSessionParam("DEVICETYPE") == "m" || $oCRNRSTN_ENV->getEnvParam('MOBILE_ONLY') == true){
    ...
    <div data-role="page" id="myPage">
        ...
        <div role="main" class="ui-content">
            ...
            <h2 style=" padding-bottom:0px; margin-bottom:0px;">Edit Page Header</h2>
            <div class="cb_10"></div>

            <form action="#" method="post" name="edit_page_header" id="edit_page_header"  enctype="multipart/form-data" data-ajax="false">

                <textarea cols="40" rows="8" name="header_copy" id="header_copy" placeholder="Enter page header text..."><?php echo $tmp_current_copy; ?></textarea>
                <div class="frm_errstatus header_copy_req" style="width:100%; display:none;">Required</div>
                <div class="frm_errstatus header_copy_invalid" style="width:100%; display:none;">Invalid header content</div>
                <div class="cb_10"></div>
                ...
                <input type="hidden" name="postid" value="edit_page_header">
            ...
            <script type="application/javascript" language="javascript">
                $( "#edit_page_header" ).submit(function( event ) {
                    //
                    // VALIDATE FORM
                    return validateForm('new_page_header');
                });

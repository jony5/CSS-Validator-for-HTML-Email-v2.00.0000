...
$tmp_serial_handle = 'SITE_ACTIVITY_LOGS';
$oDB_RESP = $oUSER->getSiteActivityLogs($tmp_serial_handle);
...
<div data-role="page" id="myPage">
    ...
    //$tmp_clientName_Header = '<a href="'.$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR').'account/dashboard/?uid='.$oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYSTEM_USERS', 'USERID').'" data-ajax="false" style="text-decoration:none;">'.$oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYSTEM_USERS', 'FIRSTNAME_BLOB')." ".$oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYSTEM_USERS', 'LASTNAME_BLOB').'</a>';
	...
	<div role="main" class="ui-content" id="myPage">
        <h2 style=" padding-bottom:0px; margin-bottom:0px;">Activity Logs</h2>
        <p style="padding-top:10px; margin-top:0px;padding-bottom:0px; margin-bottom:0px;">Recent website activity.</p>
        <?php
        $tmp_loop_size_SYS_LOGS = $oDB_RESP->return_sizeof($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS');

        for($i=0;$i<$tmp_loop_size_SYS_LOGS;$i++){
            ?>
            <p style="padding-bottom:0px; margin-bottom:0px;"><?php echo $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS', 'ACTIVITY_DESCRIPTION_BLOB', $i); ?></p>

            <div class="cb_10"></div>
            <div style="font-size:85%;">
                <div style="float:left;"><a href="#" style="font-weight: normal;" data-ajax="false">Full Screen Overlay</a></div>
                <div style="float:left; padding-left: 20px;"><a href="#" style="font-weight: normal;" data-ajax="false">Full Screen Page</a></div>
                <div style="float:left; padding-left: 20px;"><a href="#" style="font-weight: normal;" data-ajax="false">Page Component</a></div>
                <div class="cb"></div>
            </div>

            <div class="ui-nodisc-icon ui-alt-icon">
                <?php
                if($oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS', 'FULL_SCRN_PAGE_ID', $i)!=""){
                    ?>
                    <a href="<?php echo $oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR');  ?>dashboard/" class="ui-btn ui-shadow ui-corner-all ui-icon-info ui-btn-icon-notext ui-btn-inline" data-ajax="false">Details</a>
                    <?php
                }else{
                    ?>
                    <a href="<?php echo $oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP').$oCRNRSTN_ENV->getEnvParam('ROOT_PATH_CLIENT_HTTP_DIR');  ?>dashboard/" class="ui-btn ui-shadow ui-corner-all ui-icon-info ui-btn-icon-notext ui-btn-inline" data-ajax="false">Details</a>
            ...
            <h5 style="padding-top:0px; margin-top:0px;">By <?php echo $oDB_RESP->retrieveDataByID($oDB_RESP->return_serial($tmp_serial_handle), 'SYSTEM_USERS', $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS', 'MODIFIER_ID', $i), 'FIRSTNAME'); ?> <?php echo $oDB_RESP->retrieveDataByID($oDB_RESP->return_serial($tmp_serial_handle), 'USERS', $oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS', 'MODIFIER_ID', $i), 'LASTNAME'); ?> on <?php echo date("m.d.Y \a\\t H:i:s", strtotime($oDB_RESP->return_data_element($oDB_RESP->return_serial($tmp_serial_handle), 'SYS_LOGS', 'DATECREATED', $i))); ?></h5><!-- (08/12/2018 at 12:25:09)  2018-08-10 08:51:00 -->
            <hr>

...
crnrstn_fhandler.prototype = {
	...
	initialize: function() {
		...
		var inputs_textarea = document.getElementsByTagName('textarea');

		//
		// LOOP THROUGH ALL INPUT TAGS
		for (var ii=0; ii<inputs_textarea.length; ii++){
			input_textarea = inputs_textarea[ii];

			if($(input_textarea.id+'_input_validation_copy')){
				//
				// MANAGE BEHAVIOR VIA EFFECTS
				$(input_textarea.id+'_input_validation_copy').hide();
			}

			if(ii==0 && $(input_textarea.id)){
				$(input_textarea.id).focus();
			}

			var frm_initAttribute2 = String(input_textarea.getAttribute('frm_init'));

			//
			// USE THE string.match() METHOD TO CATCH 'crnrstn_frm_handle' REFERENCES IN THE frm_init ATTRIBUTE
			if (input_textarea.getAttribute('crnrstn_frm_valtype') && (frm_initAttribute2.toLowerCase().match('crnrstn_frm_handle'))){
				crnrstn_inputIDArray[ii] = input_textarea;
				input_textarea.onclick = function () {mycrnrstn_fhandler.clickListener(this);}
				input_textarea.onkeydown = function () {mycrnrstn_fhandler.keyboardListener(this); }
				//input_textarea.onblur = function () {mycrnrstn_fhandler.leavingListener(this);}  // throws error
			}
	...
	hideErrorInFull: function(formElem){
		...
		$(formElem.id+'_form_element_label').morph('color:#1A182D;', {duration: 0.5});
		$(formElem.id).morph('background-color:##D0D0E0; border:2px solid #6B5555; color:#1A182D;', {duration: 0.5});
		}

...
class user {
    ...
    public function syncXML(){
        ...
        /*
                ...
                //
                // CONSTANTS
                $SITEROOT="C:/apache/htdocs/phonelist/";

                //
                // INITIALIZE & CLOSE CLIENT NODE
                $xmlFileNEW=('<?xml version="1.0" encoding="iso-8859-1"?><employeelist>');
                $xmlFileCLOSE=('</employeelist>');
                $xmlprofileNodes=('');

                $queryDepartments=mysql_query("select id, name from departments where status='active'");
                while(list($id, $name)=mysql_fetch_row($queryDepartments)){
                    $departmentARRAY[$id]=$name;
                }

                $queryOffices=mysql_query("select id, name, displayname from office where status='active'");
                while(list($id, $name, $displayname)=mysql_fetch_row($queryOffices)){
                    $officeARRAY[$id]=$displayname;
                }

                $nid=0;
        //
        // GET CLIENT DATA
                $queryProfiles=mysql_query("select id, firstname, lastname, alias, email, filename, position, department,office, extension, mobilenumber,yahooim, thumbwidth, thumbheight from phonelist where status='active' ORDER BY firstname ASC");
                while(list($eid, $firstname, $lastname, $alias, $email, $filename, $position, $department, $office, $extension, $mobilenumber,$yahooim, $thumbwidth, $thumbheight)=mysql_fetch_row($queryProfiles)){
                    $firstname=cleanupcharacters($firstname);
                    $lastname=cleanupcharacters($lastname);
                    $position=cleanupcharacters($position);
                    $extension=cleanupcharacters($extension);
                    $mobilenumber=cleanupcharacters($mobilenumber);
                    $yahooim=cleanupcharacters($yahooim);

                    //
                    // READABLE OUTPUT
                    //	$xmlprofileNodes_temp=('
                    //		<profile>
                    //			<eid>'.$eid.'</eid>
                    ...
                    //			<department id="'.$department.'">'.$departmentARRAY[$department].'</department>
                    //			<office id="'.$office.'">'.$officeARRAY[$office].'</office>
                    //			<thumbnail width="'.$thumbwidth.'" height="'.$thumbheight.'">'.$filename.'</thumbnail>
                    //			<yahooim>'.$yahooim.'</yahooim>
                    //		</profile>
                    //	');

                    // COMPRESSED OUTPUT
                    $xmlprofileNodes_temp=('<profile><nid>'.$nid.'</nid><eid>'.$eid.'</eid><firstname>'.$firstname.'</firstname><lastname>'.$lastname.'</lastname><email>'.$email.'</email><alias>'.$alias.'</alias><position>'.$position.'</position><extension>'.$extension.'</extension><mobilenumber>'.$mobilenumber.'</mobilenumber><department id="'.$department.'">'.$departmentARRAY[$department].'</department><office id="'.$office.'">'.$officeARRAY[$office].'</office><thumbnail width="'.$thumbwidth.'" height="'.$thumbheight.'">'.$filename.'</thumbnail><yahooim>'.$yahooim.'</yahooim></profile>');

                    $xmlprofileNodes=$xmlprofileNodes.$xmlprofileNodes_temp;
                ...
                $xmlProfilesCUM=$xmlFileNEW.$xmlprofileNodes.$xmlFileCLOSE;

                $xmlFilenameabs=$SITEROOT."/xml/mphone.xml";
                $xmlFilenameabs_ADMIN=$SITEROOT."admin/xml/mphone.xml";

                //
                // UPDATE  FILE
                if(file_exists($xmlFilenameabs)){
                    //
                    // DELETE THE FILE
                    unlink($xmlFilenameabs);

                    //
                    // UPDATE XML FILE WITH NEW DATA
                    $file_handle = fopen($xmlFilenameabs,'a');
                    fwrite($file_handle,$xmlProfilesCUM);
                    fclose($file_handle);
